...
mpki_l1_Axis=np.ones(combinations)
mpki_l2_Axis=np.ones(combinations)
...
for outFile in sys.argv[2:]:
	...
	ipc_temp.append(ipc)
	mpki_temp.append(mpki_l1)
	mpki2_temp.append(mpki_l2)

	if (counter==combinations):
		ipc_Axis = np.multiply(ipc_Axis, ipc_temp)
		mpki_l1_Axis= np.multiply(mpki_l1_Axis, mpki_temp)
		mpki_l2_Axis= np.multiply(mpki_l2_Axis, mpki2_temp)
		counter=0
		ipc_temp = []
		mpki_temp = []
		mpki2_temp = []
...
for i in range(len(ipc_Axis)):
	...
	mpki_l1_Axis[i] = mpki_l1_Axis[i]**(1.0/10)
	mpki_l2_Axis[i] = mpki_l2_Axis[i]**(1.0/10)
...
plt.title("Mean Values: IPC vs MPKI_l1 vs MPKI_l2")
